...
abstract class AbstractCloudFoundryAtomicOperationConverter extends AbstractAtomicOperationsCredentialsSupport {
  ...
  void downloadAndProcessManifest(Map manifest, Consumer<Map> processManifest, ArtifactDownloader artifactDownloader) {
    Artifact manifestArtifact = getObjectMapper().convertValue(manifest.get("artifact"), Artifact.class);

    try {
      InputStream manifestInput = artifactDownloader.download(manifestArtifact);
