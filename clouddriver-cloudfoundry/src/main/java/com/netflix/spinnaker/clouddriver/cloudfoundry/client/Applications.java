...
public class Applications {
  ...
  public List<CloudFoundryApplication> getLatestApplications() {
    List<Application> newCloudFoundryAppList =
        collectPages("applications", page -> api.all(page, 5000, null, null));

    // Evict Records that are no longer in the foundation
    ...
    // if the update time doesn't match we need to update the cache
    // if the app is not found in the cache we need to process and update the cache
    ...
    // execute health check on instances, set number of available instances and health status of
    // instances
  ...
  public List<CloudFoundryApplication> all() {
    List<CloudFoundryServerGroup> serverGroups =
        collectPages("applications", page -> api.all(page, 5000, null, null)).stream()
            .map(this::map)
            .collect(toList());
